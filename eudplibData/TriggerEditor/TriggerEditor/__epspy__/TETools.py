...
def ScreenShake(x, y, tempLoc):
    ...
    if EUDIf()(IsUserCP()):
        ...
        # (Line 670) const chkt = GetChkTokenized();
        DoActions(CenterView(64))
        chkt = GetChkTokenized()
        # (Line 671) const dim = chkt.getsection("DIM");
        dim = chkt.getsection("DIM")
        # (Line 672) const mapX, mapY = b2i2(dim) * 16, b2i2(dim, 2) * 16;  // × 32 ÷ 2 (map center)
        mapX, mapY = List2Assignable([b2i2(dim) * 16, b2i2(dim, 2) * 16])
        # (Line 673) addloc(tempLoc, mapX - dwread(SCREEN_X), mapY - dwread(SCREEN_Y));
        f_addloc(tempLoc, mapX - f_dwread(SCREEN_X), mapY - f_dwread(SCREEN_Y))
        # (Line 674) CenterView(tempLoc);
        # (Line 675) }
        DoActions(CenterView(tempLoc))
        # (Line 676) }
    EUDEndIf()
    # (Line 696) function hotkeyUnitEPD(epd, player: TrgPlayer, hotkey, slot) {

@EUDTypedFunc([None, TrgPlayer, None, None])
def f_hotkeyUnitEPD(epd, player, hotkey, slot):
    # (Line 697) epd += 0xA5 / 4;
    epd.__iadd__(0xA5 // 4)
    # (Line 698) const tosread_epd = readgen_epd(0xFF00, list(0, py_eval('lambda x: 8*x')));
    tosread_epd = f_readgen_epd(0xFF00, FlattenList([0, eval('lambda x: 8*x')]))
    # (Line 699) const alphaID = tosread_epd(epd);
    alphaID = tosread_epd(epd)
    # (Line 700) if (epd == 19025 + 0xA5/4) {
    if EUDIf()(epd == 19025 + 0xA5 // 4):
        # (Line 701) DoActions(alphaID.AddNumber(1));
        DoActions(alphaID.AddNumber(1))
        # (Line 702) } else {
    if EUDElse()():
        # (Line 703) const diff = 1701 - (161866 - epd) / 84;
        diff = 1701 - (161866 - epd) // 84
        # (Line 704) VProc(diff, diff.QueueAddTo(alphaID));
        VProc(diff, diff.QueueAddTo(alphaID))
        # (Line 705) }
        # (Line 706) SetDeaths(EPD(0x57FE60) + 216 * player + slot, SetTo, alphaID, hotkey);
    EUDEndIf()
    # (Line 707) }
...
def LocationAngle(originLoc, destLoc):
    # (Line 757) const ox1, oy1, ox2, oy2 = __GetLocCoord(originLoc);
    ox1, oy1, ox2, oy2 = List2Assignable([__GetLocCoord(originLoc)])
    # (Line 758) const dx1, dy1, dx2, dy2 = __GetLocCoord(destLoc);
...
def LocationDistance(loc1, loc2):
    # (Line 784) const ox1, oy1, ox2, oy2 = __GetLocCoord(loc1);
    ox1, oy1, ox2, oy2 = List2Assignable([__GetLocCoord(loc1)])
    # (Line 785) const dx1, dy1, dx2, dy2 = __GetLocCoord(loc2);
...
def LocationDistanceVal(variable, loc1, loc2):
    ...
    # (Line 794) }
    # (Line 815) function Distance(x1, y1, x2, y2) {

@EUDFunc
def Distance(x1, y1, x2, y2):
    # (Line 816) const x = x2 - x1;
    x = x2 - x1
    # (Line 817) const y = y2 - y1;
    y = y2 - y1
    # (Line 818) return sqrt(x*x + y*y);
    EUDReturn(f_sqrt(x * x + y * y))
    # (Line 819) }
    # (Line 838) function PolarLocation(loc: TrgLocation, length, angle) {

@EUDTypedFunc([TrgLocation, None, None])
def PolarLocation(loc, length, angle):
    # (Line 839) const x, y = lengthdir(length, angle);
    x, y = List2Assignable([f_lengthdir(length, angle)])
    # (Line 840) addloc(loc, x, y);
    f_addloc(loc, x, y)
    # (Line 841) }
    # (Line 864) function CheckNoneTargetSkillEPD(

# (Line 865) unitEPD,
# (Line 866) range,
# (Line 867) unitLoc: TrgLocation,
# (Line 868) targetLoc: TrgLocation,
# (Line 869) skillOrder /*Order*/
# (Line 870) ) {
...
def CheckNoneTargetSkillEPD(unitEPD, range, unitLoc, targetLoc, skillOrder):
    # (Line 871) unitEPD += 0x4D/4;
    unitEPD.__iadd__(0x4D // 4)
    # (Line 872) if (MemoryXEPD(unitEPD, Exactly, skillOrder * 256, 0xFF00)) {
    if EUDIf()(MemoryXEPD(unitEPD, Exactly, skillOrder * 256, 0xFF00)):
        # (Line 873) unitEPD += -((0x4D - 0x28)/4);
        unitEPD.__iadd__(-((0x4D - 0x28) // 4))
        # (Line 874) const x, y = posread_epd(unitEPD);
        x, y = List2Assignable([f_posread_epd(unitEPD)])
        # (Line 875) setloc(unitLoc, x, y);
        f_setloc(unitLoc, x, y)
        # (Line 876) unitEPD += (0x58 - 0x28)/4;
        unitEPD.__iadd__((0x58 - 0x28) // 4)
        # (Line 877) setloc_epd(targetLoc, unitEPD);
        f_setloc_epd(targetLoc, unitEPD)
        # (Line 879) if (range > LocationDistance(unitLoc, targetLoc)) {
        if EUDIf()(range <= LocationDistance(unitLoc, targetLoc), neg=True):
            # (Line 880) MoveLocation(targetLoc, 227, AllPlayers, targetLoc);
            # (Line 881) unitEPD += -((0x58 - 0x4C)/4);
            DoActions(MoveLocation(targetLoc, 227, AllPlayers, targetLoc))
            unitEPD.__iadd__(-((0x58 - 0x4C) // 4))
            # (Line 882) SetMemoryXEPD(unitEPD, SetTo, 0x100, 0xFF00);
            # (Line 883) unitEPD += -((0x4D - 0x10)/4);
            DoActions(SetMemoryXEPD(unitEPD, SetTo, 0x100, 0xFF00))
            unitEPD.__iadd__(-((0x4D - 0x10) // 4))
            # (Line 884) SetMemoryEPD(unitEPD, SetTo, x + y * 65536);
            # (Line 885) return True;
            DoActions(SetMemoryEPD(unitEPD, SetTo, x + y * 65536))
            EUDReturn(True)
            # (Line 886) }
            # (Line 887) }
        EUDEndIf()
        # (Line 888) return False;
    EUDEndIf()
    EUDReturn(False)
    # (Line 889) }
    # (Line 912) function CheckNoneTargetSkill(

# (Line 913) unitPTR,
# (Line 914) range,
# (Line 915) unitLoc: TrgLocation,
# (Line 916) targetLoc: TrgLocation,
# (Line 917) skillOrder /*Order*/
# (Line 918) ) {
@EUDTypedFunc([None, None, TrgLocation, TrgLocation, None])
def CheckNoneTargetSkill(unitPTR, range, unitLoc, targetLoc, skillOrder):
    # (Line 919) return CheckNoneTargetSkillEPD(
    # (Line 920) EPD(unitPTR),
    # (Line 921) range,
    # (Line 922) unitLoc,
    # (Line 923) targetLoc,
    # (Line 924) skillOrder /*Order*/
    # (Line 925) );
    EUDReturn(CheckNoneTargetSkillEPD(EPD(unitPTR), range, unitLoc, targetLoc, skillOrder))
    # (Line 926) }
    # (Line 945) function SetLocationToUnitSafeEPD(location: TrgLocation, unitEPD, posX) {

@EUDTypedFunc([TrgLocation, None, None])
def SetLocationToUnitSafeEPD(location, unitEPD, posX):
    # (Line 946) unitEPD += posX/4;
    unitEPD.__iadd__(posX // 4)
    # (Line 947) setloc_epd(location, unitEPD);
    f_setloc_epd(location, unitEPD)
    # (Line 948) }
    # (Line 966) function SetLocationToUnitSafe(location: TrgLocation, unitPTR, posX) {

@EUDTypedFunc([TrgLocation, None, None])
def SetLocationToUnitSafe(location, unitPTR, posX):
    # (Line 967) SetLocationToUnitSafeEPD(location, EPD(unitPTR), posX);
    SetLocationToUnitSafeEPD(location, EPD(unitPTR), posX)
    # (Line 968) }
    # (Line 991) function CheckTargetSkillEPD(

# (Line 992) unitEPD,
# (Line 993) range,
# (Line 994) unitLoc: TrgLocation,
# (Line 995) targetLoc: TrgLocation,
# (Line 996) skillOrder
# (Line 997) ) {
@EUDTypedFunc([None, None, TrgLocation, TrgLocation, None])
def CheckTargetSkillEPD(unitEPD, range, unitLoc, targetLoc, skillOrder):
    # (Line 998) unitEPD += 19;  // 0x4D orderID
    unitEPD.__iadd__(19)
    # (Line 999) if (MemoryXEPD(unitEPD, Exactly, skillOrder * 256, 0xFF00)) {
    if EUDIf()(MemoryXEPD(unitEPD, Exactly, skillOrder * 256, 0xFF00)):
        # (Line 1000) unitEPD += 4;  // 0x5C orderTargetUnit
        unitEPD.__iadd__(4)
        # (Line 1001) if (MemoryEPD(unitEPD, Exactly, 0)) return False;
        if EUDIf()(MemoryEPD(unitEPD, Exactly, 0)):
            EUDReturn(False)
            # (Line 1002) unitEPD -= 1;  // 0x58 orderTargetPos
        EUDEndIf()
        unitEPD.__isub__(1)
        # (Line 1003) const tx, ty = posread_epd(unitEPD);
        tx, ty = List2Assignable([f_posread_epd(unitEPD)])
        # (Line 1004) unitEPD -= 12;  // 0x28 position
        unitEPD.__isub__(12)
        # (Line 1005) const x, y = posread_epd(unitEPD);
        x, y = List2Assignable([f_posread_epd(unitEPD)])
        # (Line 1006) const dx, dy = tx - x, ty - y;
        dx, dy = List2Assignable([tx - x, ty - y])
        # (Line 1007) if (dx*dx + dy*dy < range*range) {
        if EUDIf()(dx * dx + dy * dy >= range * range, neg=True):
            # (Line 1008) setloc(unitLoc, x, y);
            f_setloc(unitLoc, x, y)
            # (Line 1009) setloc(targetLoc, tx, ty);
            f_setloc(targetLoc, tx, ty)
            # (Line 1010) DoActions(list(
            # (Line 1011) unitEPD.SubtractNumber(6),  // 0x10 moveTarget
            # (Line 1012) RemoveUnitAt(1, 101, 255, P9),
            # (Line 1013) ));
            DoActions(FlattenList([unitEPD.SubtractNumber(6), RemoveUnitAt(1, 101, 255, P9)]))
            # (Line 1014) SetMemoryEPD(unitEPD, SetTo, x + y * 65536);
            # (Line 1015) unitEPD += 9;  // 0x4D orderID
            DoActions(SetMemoryEPD(unitEPD, SetTo, x + y * 65536))
            unitEPD.__iadd__(9)
            # (Line 1016) SetMemoryXEPD(unitEPD, SetTo, 0x100, 0xFF00);
            # (Line 1017) return True;
            DoActions(SetMemoryXEPD(unitEPD, SetTo, 0x100, 0xFF00))
            EUDReturn(True)
            # (Line 1018) }
            # (Line 1019) }
        EUDEndIf()
        # (Line 1020) return False;
    EUDEndIf()
    EUDReturn(False)
    # (Line 1021) }
    # (Line 1044) function CheckTargetSkill(

# (Line 1045) unitPTR,
# (Line 1046) range,
# (Line 1047) unitLoc: TrgLocation,
# (Line 1048) targetLoc: TrgLocation,
# (Line 1049) skillOrder
# (Line 1050) ) {
@EUDTypedFunc([None, None, TrgLocation, TrgLocation, None])
def CheckTargetSkill(unitPTR, range, unitLoc, targetLoc, skillOrder):
    # (Line 1051) return CheckTargetSkillEPD(EPD(unitPTR), range, unitLoc, targetLoc, skillOrder);
    EUDReturn(CheckTargetSkillEPD(EPD(unitPTR), range, unitLoc, targetLoc, skillOrder))
    # (Line 1052) }
    # (Line 1069) function CBringEPD(unitEPD, location: TrgLocation, dummyUnit: TrgUnit) {

@EUDTypedFunc([None, TrgLocation, TrgUnit])
def CBringEPD(unitEPD, location, dummyUnit):
    # (Line 1070) MoveLocation(location, dummyUnit, AllPlayers, location);
    # (Line 1071) const unitType = unitEPD + 0x64/4;
    DoActions(MoveLocation(location, dummyUnit, AllPlayers, location))
    unitType = unitEPD + 0x64 // 4
    # (Line 1072) const originalUnitType = wread_epd(unitType, 0);
    originalUnitType = f_wread_epd(unitType, 0)
    # (Line 1073) SetMemoryXEPD(unitType, SetTo, dummyUnit, 0xFFFF);
    # (Line 1074) const ret = l2v(Bring(AllPlayers, AtLeast, 1, dummyUnit, location));
    DoActions(SetMemoryXEPD(unitType, SetTo, dummyUnit, 0xFFFF))
    ret = _L2V(Bring(AllPlayers, AtLeast, 1, dummyUnit, location))
    # (Line 1075) SetMemoryXEPD(unitType, SetTo, originalUnitType, 0xFFFF);
    # (Line 1076) return ret;
    DoActions(SetMemoryXEPD(unitType, SetTo, originalUnitType, 0xFFFF))
    EUDReturn(ret)
    # (Line 1077) }
    # (Line 1094) function CBring(unitPTR, location: TrgLocation, dummyUnit: TrgUnit) {

@EUDTypedFunc([None, TrgLocation, TrgUnit])
def CBring(unitPTR, location, dummyUnit):
    # (Line 1095) return CBringEPD(EPD(unitPTR), location, dummyUnit);
    EUDReturn(CBringEPD(EPD(unitPTR), location, dummyUnit))
    # (Line 1096) }
    # (Line 1107) function BuildResetEPD(unitEPD) {

@EUDFunc
def BuildResetEPD(unitEPD):
    # (Line 1108) VProc(unitEPD, list(
    # (Line 1109) unitEPD.AddNumber(0x98/4),
    # (Line 1110) unitEPD.SetDest(EPD(0x6509B0)),
    # (Line 1111) ));
    VProc(unitEPD, FlattenList([unitEPD.AddNumber(0x98 // 4), unitEPD.SetDest(EPD(0x6509B0))]))
    # (Line 1112) setcurpl2cpcache(actions=list(
    # (Line 1113) SetDeaths(CurrentPlayer, SetTo, 0xE400E4, 0),
    # (Line 1114) SetMemory(0x6509B0, Add, 1),
    # (Line 1115) SetDeaths(CurrentPlayer, SetTo, 0xE400E4, 0),
    # (Line 1116) SetMemory(0x6509B0, Add, 1),
    # (Line 1117) SetDeathsX(CurrentPlayer, SetTo, 228, 0, 0xFFFF),
    # (Line 1118) SetMemory(0x6509B0, Add, 1),
    # (Line 1119) SetDeathsX(CurrentPlayer, SetTo, 2 << 16, 0, 0xFFFF0000),
    # (Line 1120) ));
    ...
    # (Line 1121) }
    # (Line 1132) function BuildReset(unitPTR) {

@EUDFunc
def BuildReset(unitPTR):
    # (Line 1133) BuildResetEPD(EPD(unitPTR));
    BuildResetEPD(EPD(unitPTR))
    # (Line 1134) }
    # (Line 1148) function BuildCheckEPD(unitEPD, unit: TrgUnit) {

@EUDTypedFunc([None, TrgUnit])
def BuildCheckEPD(unitEPD, unit):
    # (Line 1149) unitEPD += 0x98 / 4;  // buildQueue
    unitEPD.__iadd__(0x98 // 4)
    # (Line 1150) if (MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)) {
    if EUDIf()(MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)):
        # (Line 1151) return True;
        EUDReturn(True)
        # (Line 1152) } else {
    if EUDElse()():
        # (Line 1153) const mulUnit = unit * 65536;
        mulUnit = unit * 65536
        # (Line 1154) if (MemoryXEPD(unitEPD, Exactly, mulUnit, 0xFFFF0000)) {
        ...
        if EUDElse()():
            # (Line 1157) unitEPD += 1;
            unitEPD.__iadd__(1)
            # (Line 1158) if (MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)) {
            if EUDIf()(MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)):
                # (Line 1159) return True;
                EUDReturn(True)
                # (Line 1160) } else if (MemoryXEPD(unitEPD, Exactly, mulUnit, 0xFFFF0000)) {
            ...
            if EUDElse()():
                # (Line 1163) unitEPD += 1;
                unitEPD.__iadd__(1)
                # (Line 1164) if (MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)) {
                if EUDIf()(MemoryXEPD(unitEPD, Exactly, unit, 0xFFFF)):
                    # (Line 1165) return True;
                    EUDReturn(True)
                    # (Line 1166) }
                    # (Line 1167) }
                EUDEndIf()
            ...
            # (Line 1169) }
        EUDEndIf()
        # (Line 1170) return False;
    EUDEndIf()
    EUDReturn(False)
    # (Line 1171) }
    # (Line 1185) function BuildCheck(unitPTR, unit: TrgUnit) {

@EUDTypedFunc([None, TrgUnit])
def BuildCheck(unitPTR, unit):
    # (Line 1186) return BuildCheckEPD(EPD(unitPTR), unit);
    EUDReturn(BuildCheckEPD(EPD(unitPTR), unit))
    # (Line 1187) }
    # (Line 1210) function AttackGround(

# (Line 1211) attackLoc: TrgLocation,
# (Line 1212) targetLoc: TrgLocation,
# (Line 1213) attackUnit: TrgUnit,
# (Line 1214) targetUnit: TrgUnit,
# (Line 1215) player: TrgPlayer
# (Line 1216) ) {
...
def AttackGround(attackLoc, targetLoc, attackUnit, targetUnit, player):
    # (Line 1217) if (Memory(0x628438, Exactly, 0)) return;  // avoid EUDError when CCMU
    if EUDIf()(Memory(0x628438, Exactly, 0)):
        EUDReturn()
        # (Line 1218) const aUnit = cunitepdread_epd(EPD(0x628438))[[1]];
    EUDEndIf()
    aUnit = f_cunitepdread_epd(EPD(0x628438))[1]
    # (Line 1219) CreateUnit(1, attackUnit, attackLoc, player);
    # (Line 1220) DoActions(aUnit.AddNumber(0x110 / 4));
    DoActions(CreateUnit(1, attackUnit, attackLoc, player))
    DoActions(aUnit.AddNumber(0x110 // 4))
    # (Line 1221) SetMemoryEPD(aUnit, Add, 2);  // aUnit + 0x110 (CUnit::removeTimer)
    # (Line 1223) if (Memory(0x628438, Exactly, 0)) return;  // avoid EUDError when CCMU
    DoActions(SetMemoryEPD(aUnit, Add, 2))
    if EUDIf()(Memory(0x628438, Exactly, 0)):
        EUDReturn()
        # (Line 1224) const tunitptr, tunit = cunitepdread_epd(EPD(0x628438));
    EUDEndIf()
    tunitptr, tunit = List2Assignable([f_cunitepdread_epd(EPD(0x628438))])
    # (Line 1225) CreateUnit(1, targetUnit, targetLoc, player);
    # (Line 1226) DoActions(list(
    DoActions(CreateUnit(1, targetUnit, targetLoc, player))
    # (Line 1227) tunit.AddNumber(0x110 / 4),
    # (Line 1228) aUnit.AddNumber(-((0x110 - 0x5C) / 4))
    # (Line 1229) ));
    DoActions(FlattenList([tunit.AddNumber(0x110 // 4), aUnit.AddNumber(-((0x110 - 0x5C) // 4))]))
    # (Line 1230) SetMemoryEPD(aUnit, SetTo, tunitptr);  // aUnit + 0x5C (CUnit::targetUnit)
    # (Line 1231) SetMemoryEPD(tunit, Add, 2);  // tunit + 0x110 (CUnit::removeTimer)
    DoActions(SetMemoryEPD(aUnit, SetTo, tunitptr))
    # (Line 1233) DoActions(aUnit.AddNumber(-((0x5C - 0x4C) / 4)));
    DoActions(SetMemoryEPD(tunit, Add, 2))
    DoActions(aUnit.AddNumber(-((0x5C - 0x4C) // 4)))
    # (Line 1234) SetMemoryXEPD(aUnit, SetTo, 0xA00, 0xFF00);  // order[10]=attack, 0x4D (CUnit::order)
    # (Line 1235) }
    DoActions(SetMemoryXEPD(aUnit, SetTo, 0xA00, 0xFF00))
    # (Line 1249) function AddStatusFlagsEPD(epd, flags) {

@EUDFunc
def AddStatusFlagsEPD(epd, flags):
    # (Line 1250) epd += 0xDC / 4;
    epd.__iadd__(0xDC // 4)
    # (Line 1251) SetMemoryXEPD(epd, SetTo, ~0, flags);
    # (Line 1252) }
    DoActions(SetMemoryXEPD(epd, SetTo, ~0, flags))
    # (Line 1266) function AddStatusFlags(unitPTR, flags) {

@EUDFunc
def AddStatusFlags(unitPTR, flags):
    # (Line 1267) unitPTR += 0xDC;
    unitPTR.__iadd__(0xDC)
    # (Line 1268) SetMemoryX(unitPTR, SetTo, ~0, flags);
    # (Line 1269) }
    DoActions(SetMemoryX(unitPTR, SetTo, ~0, flags))
    # (Line 1286) function AddSizeLocation(location: TrgLocation, width, height) {

@EUDTypedFunc([TrgLocation, None, None])
def AddSizeLocation(location, width, height):
    # (Line 1287) dilateloc(location, width, height);
